...
template <class K> // This template function finds the right track
void check_lane(bool &lane1, bool &lane2, bool check_two_lanes, K const& sensor_fusion, double car_s){
    if(check_two_lanes){
        for (int n = 0; n < sensor_fusion.size(); n++) {
            double d = sensor_fusion[n][6]; //check ith car's d value
            double check_car_s = sensor_fusion[n][5];
            if (d > 0 && d < 4) // car is in Left lane
            {
                if ((check_car_s > (car_s - 20)) && (check_car_s < (car_s + 20))) lane1 = false;
            } else if (check_two_lanes && d > 8 && d < 12) // car is in Right lane
            {
                if ((check_car_s > (car_s - 20)) && (check_car_s < (car_s + 20))) lane2 = false;
            }
        }
    }
    else{
        for (int n = 0; n < sensor_fusion.size(); n++) {
            double d = sensor_fusion[n][6]; //check ith car's d value
            double check_car_s = sensor_fusion[n][5];
            if (d > 4 && d < 8) // car is in Left lane
            {
                if ((check_car_s > (car_s - 20)) && (check_car_s < (car_s + 20))) lane1 = false;
            }
        }
        ...
        if (length && length > 2 && data[0] == '4' && data[1] == '2') {
            ...
            if (s != "") {
                ...
                if (event == "telemetry") {
                    ...
                    for (int i = 0; i < sensor_fusion.size(); i++)
                    {
                        ...
                        if (d < (2+4*lane+2) && d > (2+4*lane-2))
                        {
                            ...
                            if ((check_car_s > car_s) && ((check_car_s-car_s) < 30))
                            {
                                ...
                                if (lane == 1)
                                {
                                    check_lane(leftlanechange_allowed, rightlanechange_allowed,true,sensor_fusion,car_s);
                                    if(leftlanechange_allowed) lane = 0;
                                    if(rightlanechange_allowed) lane = 2;

                                }

                                if(lane == 0)
                                {
                                    check_lane(rightlanechange_allowed,leftlanechange_allowed,false,sensor_fusion,car_s);

                                    if(rightlanechange_allowed) lane = 1; // shift to center lane.
                                }

                                if(lane == 2)
                                {
                                    check_lane(leftlanechange_allowed,leftlanechange_allowed,false,sensor_fusion,car_s);

                                    if(leftlanechange_allowed) lane = 1;
                                }
                            }
                        }
                    ...
                    if (too_close) ref_vel -= 0.424;
                    else if(ref_vel < 49.5) ref_vel += 0.325;

                    vector<double> ptsx;
                    vector<double> ptsy;

                    // Create reference x,y,yaw state
                    // Either we will use the current car's x,y & yaw as ref or if there are enough previous points, we will use
                    // those points
